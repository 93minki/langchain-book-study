...
prompt3 = PromptTemplate.from_template(
    "다음 숙박 시설 리뷰를 읽고 0부터 10점 사이에서 부정/긍정 점수를 매기세요. 숫자만 대답하세요\n\n{translation}"
)
...
# 각 단계는 '프롬프트 | LLM | 출력 파서'의 형태로 구성된다.
# 단계 1: 리뷰 번역 체인
...
# 중간 결과들을 딕셔너리에 누적한다.
combined_lcel_chain = RunnablePassthrough.assign(
    # 입력: {'review': '원본 리뷰 텍스트'}
    # translate_chain_component는 'reivew' 키를 사용하여 호출된다.
    ...
    # summarize_chain_component와 sentiment_score_chain_component는 'translation' 키를 사용한다.
    # language_chain_component는 'review' 키를 사용한다.
    # 출력: {'review': '...', 'translation': '번역된 텍스트', 'summary': '요약된 텍스트', 'sentiment_score': '감성 점수', 'language': '언어'}
    summary=lambda x: summarize_chain_component.invoke(
        {"translation": x["translation"]}
    ),
    sentiment_score=lambda x: sentiment_score_chain_component.invoke(
        {"translation": x["translation"]}
    ),
    language=lambda x: language_chain_component.invoke({"review": x["review"]}),
) | RunnablePassthrough.assign(
  # 입력: {'review': '...', ..., 'language': '언어', 'summary': '요약된 텍스트'}
  # reply1_chain_component는 'language'와 'summary' 키를 사용한다.
  # 출력: {'review': '...', ..., 'language': '언어', 'summary': '요약된 텍스트', 'reply1': '첫 번째 답변'}
  reply1 = lambda x: reply1_chain_component.invoke({"language": x["language"], "summary": x["summary"]})
) | RunnablePassthrough.assign(
  # 입력: {'review': '...', ..., 'reply1': '첫 번째 답변'}
  # reply2_chain_component는 'reply1' 키를 사용한다 
  # 출력: {'review': '...', ..., 'reply1': '첫 번째 답변', 'reply2': '두 번째 답변'}
  reply2 = lambda x: reply2_chain_component.invoke({"reply1": x["reply1"]})
)

# 숙박 시설 리뷰 입력
review_text = """
Ein heimelig eingerichtetes kleines Zimmer mit Blick auf den Brunnen und Cafés in einer inspirierenden Familienwohnung mit entspannter Atmosphäre. Die Kommunikation war super hilfreich, sehr freundlich und klar. Metro 400 m entfernt, mit 2 zentralen Linien kommt man überall hin, die grüne Lunge bois de Vincennes nur wenige Stationen entfernt. Ich habe mich mit Katze Patti sehr wohlgefühlt und komme gern wieder.
"""
