...
write = Task(
    ...
    expected_output=(
        "마크다운 형식의 잘 작성된 블로그 게시물로, 각 섹션은 2~3개의 단락으로 구성되어 있으며, 출판 준비가 되어있습니다."
    ),
    agent=writer,
)

edit = Task(
    description=("주어진 블로그 게시물을 문법 오류와 브랜드의 특징에 맞게 교정합니다."),
    expected_output=(
...
translate_task = Task(
    description=(
        "주제에 대해 연구원이 작성해준 보고서를 기반으로 2단락 5000자로 요약한 후 한국어 콘텐츠를 작성합니다."
    ),
...
inputs = {
    "topic": "다중 에이전트 시스템 구축을 위한 LangGraph, Autogen 및 Crewai 비교 연구"
}
